...
checkCentosSELinux() {
    if command -v getenforce >/dev/null 2>&1 && [ "$(getenforce)" == "Enforcing" ]; then
        ...
        echoContent yellow "https://example.com/archives/1684115970026#centos-%E5%85%B3%E9%97%ADselinux"
        exit 0
    fi
}
checkSystem() {
...
allowPort() {
    ...
    if command -v dpkg >/dev/null 2>&1 && dpkg -l | grep -q "^[[:space:]]*ii[[:space:]]\+ufw"; then
        ...
        local updateFirewalldStatus=
        if ! iptables -L | grep -q "$1/${type}(wei3470231)"; then
            updateFirewalldStatus=true
            iptables -I INPUT -p "${type}" --dport "$1" -m comment --comment "allow $1/${type}(wei3470231)" -j ACCEPT
        fi
...
initDNSAPIConfig() {
    if [[ "$1" == "cloudflare" ]]; then
        echoContent yellow "\n CF_Token参考配置教程：https://example.com/archives/1701160377972\n"
...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            if [[ "${release}" == "alpine" ]]; then
                wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
            else
                wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
            fi
    ...
    if [[ -z "${portHoppingRange}" ]]; then
        ...
        if [[ -z "${portStart}" || -z "${portEnd}" ]]; then
            ...
            if [[ "${release}" == "centos" ]]; then
                ...
                iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "wei3470231_${type}_portHopping" -j DNAT --to-destination ":${targetPort}"
                sudo netfilter-persistent save
                if ! iptables-save | grep -q "wei3470231_${type}_portHopping"; then
                    echoContent red " ---> 端口跳跃添加失败"
                    exit 0
                fi
            fi
...
readPortHopping() {
    ...
    if [[ "${release}" == "centos" ]]; then
        ...
        if iptables-save | grep -q "wei3470231_${type}_portHopping"; then
            local portHopping=
            portHopping=$(iptables-save | grep "wei3470231_${type}_portHopping" | cut -d " " -f 8)
...
deletePortHoppingRules() {
    ...
    if [[ "${release}" == "centos" ]]; then
        ...
        iptables -t nat -L PREROUTING --line-numbers | grep "wei3470231_${type}_portHopping" | awk '{print $1}' | while read -r line; do
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        if [[ "${release}" == "alpine" ]]; then
            wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        else
            wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        fi
...
manageCDN() {
    ...
    if echo "${currentInstallProtocolType}" | grep -qE ",1,|,2,|,3,|,5,|,11,"; then
        ...
        echoContent yellow "# 注意事项"
        echoContent yellow "\n教程地址:"
        echoContent skyBlue "https://example.com/archives/cloudflarezi-xuan-ip"
...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/install.sh"
    else
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/install.sh"
    fi
    ...
    echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:wei3470231"; then
            ...
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
                ...
                if [[ -z "${showStatus}" ]]; then
                    ...
                    if [[ -f "/etc/v2ray-agent/subscribe_local/sing-box/${email}" ]]; then
                        ...
                        if [[ "${release}" == "alpine" ]]; then
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/documents/sing-box.json"
                        else
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/wei3470231/v2ray-agent/master/documents/sing-box.json"
                        fi
